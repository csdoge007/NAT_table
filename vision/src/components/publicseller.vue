...
export default {
  ...
  created () {
    //在组件创建完成之后 进行回调函数的注册
    this.$socket.registerCallBack(this.foo + 'Data',this.getData)
  },
  mounted () {
    this.initChart();
    //this.getData();
  ...
  destroyed (){
    ...
    //在组件销毁的时候，进行回调函数的取消
    this.$socket.unRegisterCallBack(this.foo + 'Data')
  },
  methods: {
    //初始化echartInstance对象
    initChart(){
      ...
      const initOption = {
         title: {
                text: ' ▎' + this.sname + '今日核酸检测人数统计', //' ▎各站点今日核酸检测人数统计'
                left: 20,
                top: 20
            },
            grid: {
